 ...
 *
 * /CLAIMS/physical_query_plan/physical_join.cpp
 *
...
PhysicalJoin::PhysicalJoin(State state)
    : state_(state),
      hash_(0),
      hashtable_(0),
...
bool PhysicalJoin::Open(const PartitionOffset& partition_offset) {
#ifdef TIME
  startTimer(&timer);
#endif
...
#endif
      const void* key_addr =
          input_schema->getColumnAddress(state_.join_index_left_[0], cur);
...
bool PhysicalJoin::Next(BlockStreamBase* block) {
  void* result_tuple;
      ...
      unsigned bn =
          state_.input_schema_right_->getcolumn(state_.join_index_right_[0])
              .operate->getPartitionValue(
                  state_.input_schema_right_->getColumnAddress(
...
inline void PhysicalJoin::IsMatch(void* l_tuple_addr, void* r_tuple_addr,
                                  void* return_addr,
  ...
  for (unsigned i = 0; i < r_join_index.size(); i++) {
    void* key_in_input =
        r_schema->getColumnAddress(r_join_index[i], r_tuple_addr);
    void* key_in_hashtable =
        l_schema->getColumnAddress(l_join_index[i], l_tuple_addr);
...
inline void PhysicalJoin::IsMatchCodegen(void* l_tuple_addr, void* r_tuple_addr,
                                         void* return_addr,
                                         vector<unsigned>& l_join_index,
                                         vector<unsigned>& r_join_index,
                                         Schema* l_schema, Schema* r_schema,
...
PhysicalJoin::JoinThreadContext::~JoinThreadContext() {
...
ThreadContext* PhysicalJoin::CreateContext() {
